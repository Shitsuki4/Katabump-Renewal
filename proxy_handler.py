...
def _load_pool():
    """Load node topology (name/server/port/sni, no passwords).

    Sources, in order:
      1. POOL_JSON env / GitHub secret: same JSON array inline.
      2. POOL_FILE env (default pool.json) path, if the file exists.

    pool.json is intentionally NOT committed to the repo — it contains
    node addresses. Provide it via the POOL_JSON secret instead.
    """
    inline = os.environ.get("POOL_JSON", "").strip()
    if inline:
        try:
            data = json.loads(inline)
            return [n for n in data if n.get("server") and n.get("port")]
        except Exception as e:
            print(f"  POOL_JSON parse failed: {e}")
            return []
